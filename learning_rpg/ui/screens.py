...
class DashboardScreen(Page):
    ...
    def __init__(self, engine):
        ...
        subtitle = QLabel("Your next small, verifiable win is waiting.")
...
class PathsScreen(Page):
    def __init__(self, engine):
        ...
        title_row.addWidget(add)
        outer.addLayout(title_row)
        note = QLabel("Keep one main path and, at most, one smaller side path active when possible.")
...
class ShopScreen(Page):
    def __init__(self, engine):
        ...
        header.addWidget(add)
        outer.addLayout(header)
        note = QLabel("Rewards are extra treats—not permission for normal rest, relationships, sleep, or basic needs.")
        note.setObjectName("muted")
...
class SettingsScreen(Page):
    ...
    def __init__(self, engine):
        ...
        about_layout.addWidget(QLabel("LEARNING RPG 1.0"))
